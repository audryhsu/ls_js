...
function random(array, n) {
  ...
  let elements = array.slice(); //create copy of input array
  let randomElements = [];

  // loop n times while you have element choices
  while (n > 0 && elements.length > 0) {
    let randomIndex = Math.floor(Math.random() * elements.length);
    let randomElement = elements[randomIndex];

    randomElements.push(randomElement);
    elements.splice(randomIndex, 1); //removing element at randomIndex from ingredients.
    n--;
  }

  return randomElements; //array of random ingredients
}
...
let dishName = random(adjective).concat(random(firstNoun), random(secondNoun)); // + concatenates the string elms of arrays
let dish = random(ingredients, 3).concat(random(spices, 2), random(extras, 1));

console.log('How about: ' + dishName.join(' ')); //dishname is a string, cannot be joined.
